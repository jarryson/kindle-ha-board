...
def init_devices():
    for k_name, d_cfg in cfg["devices"].items():
        ...
        if MusicCls and DefaultCls:
            ...
            device_managers[k_name] = Coordinator(k_name, d_cfg, m_board, d_board, d_cfg.get("timeout", 300))
...
def get_image(k_name):
    data = RAM.store.get(k_name)
    return Response(data["img"], mimetype='image/png') if data and data["img"] else ("Not Found", 404)
...
async def ha_worker():
    ...
    while True:
        try:
            async with websockets.connect(url) as ws:
                ...
                async for msg in ws:
                    ...
                    if d.get("id") == 2:
                        ...
                        for k, c in device_managers.items(): process_update(k, c, force=True)
                    if d.get("type") == "event":
            ...
            log("WS_ERR", e); await asyncio.sleep(5)
